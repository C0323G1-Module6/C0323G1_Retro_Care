...
function CreateInvoice() {
  ...
  const setInvoiceInfo = async () => {
    const elements = await elementRef.current.getElementsByClassName(
      "realPrice"
    );
    ...
    if (totalPrice - document.getElementById("paid").value < 0)
      ...
      document.getElementById("extant").value =
        totalPrice - document.getElementById("paid").value;
  };

  const setInvoiceDetailInfo = async (value, index) => {
    let quantity = document.getElementById(
      `invoiceDetailDtoSet.${index}.medicineQuantity`
    ).value;
    let discount = document.getElementById(
      `invoiceDetailDtoSet.${index}.discount`
    ).value;
    let unit = await getUnit(value.id);
    let unitPrice = parseInt(
      value.price - (value.price * (value.retailProfits + value.vat)) / 100
    );
  ...
  const dataSupllier = supplier.map((item) => ({
    label: item.code,
    value: JSON.stringify(item),
  }));
  const dataMedicine = medicine.map((item) => ({
    label: item.name,
    value: JSON.stringify(item),
  }));
  if (employee.id == null) return null;
      ...
      <div className="container">
        <div className="row">
          <div className="col-12 my-3 text-center">
            <h1 style={{ color: "#0d6efd" }}>TẠO HÓA ĐƠN NHẬP KHO</h1>
          </div>
        </div>
        <Formik
          initialValues={{
          ...
          validationSchema={Yup.object({
            ...
            note: Yup.string().max(
              100,
              "Trường nhập vào phải nhỏ hơn 100 kí tự"
            ),
            documentNumber: Yup.string()
              .required("Không được để trống trường này")
              .max(10, "Trường nhập vào phải nhỏ hơn 10 kí tự"),
            ...
            invoiceDetailDtoSet: Yup.array().of(
              Yup.object().shape({
                ...
                lot: Yup.string()
                  .required("Không được để trống trường này")
                  .max(20, "Trường nhập vào phải nhỏ hơn 20 kí tự"),
                expiry: Yup.date()
                  .required("Không được để trống trường này")
                  .min(new Date(), "Ngày phải lớn hơn ngày hiện tại"),
              })
            ),
          })}
        >
                ...
                {({ remove, push }) => (
                  <>
                    <div className="row">
                      <div className="col-8">
                        <fieldset
                          className="border border-dark rounded-3 p-3 h-100 shadow-sm"
                                ...
                                <div className="col-sm-8">
                                  <div className="row g-1">
                                        ...
                                        onChange={(value) => {
                                          if (value !== null) {
                                            ...
                                            document.getElementById(
                                              "supplierId"
                                            ).value = supplierObject.id;
                                            document.getElementById(
                                              "supplierName"
                                            ).value = supplierObject.name;
                                            document.getElementById(
                                              "supplierAddress"
                                            ).value = supplierObject.address;
                                          }
                                        }}
                                        defaultValue="0"
                                        locale={{
                                    ...
                                    <div className="col-2 h-auto p-0">
                                      <Link
                                        to={
                                          "/dashboard/supplier/create-supplier"
                                        }
                                      >
                                        <button
                                          type="button"
                                          className="w-100 btn btn-outline-primary float-end"
                                        >
                                          <FaPlus />
                                        </button>
                                      </Link>
                            ...
                            <div className="col-5">
                              ...
                              <div className="mb-3 row">
                                ...
                                <div className="col-sm-8">
                                  <input
                                    type="text"
                                    defaultValue={employee.nameEmployee}
                                    disabled
                                    className="form-control"
                                    id="input7"
                                  />
                      ...
                      <div className="col-4">
                        <fieldset
                          className="border border-dark rounded-3 p-3 shadow-sm"
                          style={{ backgroundColor: "#f8f9fa" }}
                        >
                          <legend className="float-none w-auto px-3">
                            Thanh toán
                    ...
                    <div className="row">
                      <div className="col-12">
                        ...
                        >
                                    ...
                                    (value, index) => (
                                      ...
                                      >
                                        <td style={{ width: 224 }}>
                                          ...
                                          />
                                          <Field
                                            innerRef={fieldNameRef}
                                            value={
                                              values.invoiceDetailDtoSet[index]
                                                .medicineId
                                            }
                                            hidden
                                            name={`invoiceDetailDtoSet.${index}.medicineId`}
                                          />
                                          <ErrorMessage
                                            style={{ color: "red" }}
                                            component="span"
                                        ...
                                        <td style={{ width: 120 }}>
                                          <Field
                                            ...
                                            id={`invoiceDetailDtoSet.${index}.medicineQuantity`}
                                            name={`invoiceDetailDtoSet.${index}.medicineQuantity`}
                                            class="form-control"
                                          />
                                          <ErrorMessage
                                            style={{ color: "red" }}
                                        ...
                                        >
                                          <Field
                                            type="number"
                                            id={`invoiceDetailDtoSet.${index}.discount`}
                                            onKeyUp={() =>
                                              updateInoviceDetail(
                                                values.invoiceDetailDtoSet[
                                                  index
                                            ...
                                            name={`invoiceDetailDtoSet.${index}.discount`}
                                            class="form-control"
                                          />
                                          <ErrorMessage
                                            style={{ color: "red" }}
                                        ...
                                        <td style={{ width: 224 }}>
                                          <Field
                                            type="text"
                                            name={`invoiceDetailDtoSet.${index}.lot`}
                                            class="form-control"
                                          />
                                          <ErrorMessage
                                            style={{ color: "red" }}
                                        ...
                                        >
                                          <Field
                                            type="date"
                                            name={`invoiceDetailDtoSet.${index}.expiry`}
                                            class="form-control"
                                          />
                                          <ErrorMessage
                                            style={{ color: "red" }}
                          ...
                          >
                            {" "}
                            <FaPlus className="mx-1" /> Thêm thuốc
                          </button>
                        </fieldset>
